...
**5. Nextcloud lower-cases and trims an email address before storing it.** The fingerprint a stored code is bound to does the same, so an address differing only in case or surrounding space does not read as a changed one. Both setters normalize (`lib/private/User/User.php`, `setSystemEMailAddress()` line 161 and `setPrimaryEMailAddress()` line 184 on stable33, lines 149 and 173 on stable34), and `getSystemEMailAddress()` normalizes again on the way out (line 529 on stable33, 533 on stable34). Should the two ever diverge, a code would die one send early — harmless, and named here because it would otherwise be hard to trace.
